(function() {
	...
	$.Promise.jsonFile(__dirname + "/../package.json").then(function(pkg) {
		...
		if( Opts.defaults ) {
			...
			console.log(herd)
			console.log("Git template - update:", herd.git.command({ remote: "{{remote}}", branch: "{{branch}}" }))
			process.exit(0)
		}
		...
		function Herd(herd) {
			...
			self.instanceId = [Os.hostname(), self.http.port].join("-")
			...
			Amqp.connect(self.rabbitmq.url).then(function(context) {
				...
				self.rabbitmq.publish({ op: "status", status: "online", from: self.instanceId })

				["exit", "SIGINT", "SIGTERM"].forEach(function(sig) {
					process.on(sig, function() {
						self.rabbitmq.publish({ op: "status", status: "offline", from: self.instanceId })
					})
				})
			...
			$.publish("amqp-route", self.rabbitmq.exchange, { op: "restart", arg: "all" }, function (msg) {
				self.rollingRestart()
			})
			...
			$.publish("amqp-route", self.rabbitmq.exchange, { op: "ping" }, function (msg) {
				...
				self.getStatus().then(function(status) {
					var response = { op: "pong", ts: $.now, from: self.instanceId, children: status }
		...
		Herd.prototype.setStatus = function(status) {
			self.rabbitmq.publish({ op: "status", status: status, from: self.instanceId })
		}
