(function() {
	...
	$.Promise.jsonFile(__dirname + "/../package.json").then(function(pkg) {
		...
		if( Opts.defaults ) {
			...
			console.log(herd)
			console.log("Git command template:", herd.git.command({ remote: "{{remote}}", branch: "{{branch}}" }))
			process.exit(0)
		}
		...
		function Herd(herd) {
			...
			// Restart all (or one) via AMQP message:
		...
		$.Promise.jsonFile(Opts.F).then(function(herd) {
			...
			if( Opts.nginx ) {
				console.log("upstream shepherd_pool {")
				try { for( var i = 0; i < herd.count; i++ ) {
					console.log("\tserver 127.0.0.1:"+(herd.port+i)+" max_fails=1 fail_timeout=10s;")
				} } catch( err ) {
					console.log(err)
				}
				console.log("}")
