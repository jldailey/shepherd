(function() {
	...
	$.Promise.jsonFile(__dirname + "/../package.json").then(function(pkg) {
		...
		function Child(herd, path, cmd, port, index) {
			this.herd = herd;
			this.path = path;
			this.cmd = cmd;
			this.index = index || 0;
		...
		Child.prototype.spawn = function(env) {
			var self = this,
			...
			getPortOwner(self.port).then(function(owner) {
				...
				cmd = env_string + "bash -c 'cd " + self.path + " && " + self.cmd + "'"
		...
		Child.prototype.waitForPort = function(promise) {
			waitUntilPortIsOwned(self.process.pid, self.port, self.herd.restart.timeout)
		}
		...
		function Herd(herd) {
			...
			for( i = 0; i < self.children.length; i++) {
				self.children[i] = new Child(self, self.exec.cd, self.exec.cmd, self.port, i)
			}
