var $ = require('bling'),
	...
	Herd = function Herd(opts) {
		var self = $.extend(this, Herd.defaults(opts)),
			webUpdate = null, pool = null, i = 0, j = 0;
		log("Spawning", self.exec.count, "children")
		self.children = new Array(self.exec.count);
		for( ; i < self.exec.count; i++) {
			self.children[i] = new Child(self, i)
		}

		if( self.workers.length > 0 ) {
			i = 0
			self.workers = $(self.workers).map(function(w) {
				log("Spawning", w.count, "workers")
				var a = [];
				for( j = 0; j < w.count; j++) {
					a.unshift( new Worker(w, i++) )
					a[0].spawn()
				}
				return a
			}).flatten()
		...
		// Child status via REST endpoint:
		webRoute("get", "/children", plain(function(req, res) {
			self.getChildrenStatus().then(function(list) {
		...
		webRoute("get", "/children/restart", plain(function(req, res) {
			res.send(200, "Restarting all...")
			self.rollingRestart()
		}))
		...
		webRoute("post", "/children/update", webUpdate) // listen for POST too, to make webhook support trivial
		...
		rabbitRoute({ op: "ping" }, function (msg) {
			self.getChildrenStatus().then(function(status) {
				status = { children: status, workers: self.getWorkerStatus() }
...
Herd.defaults = function(opts) {
	...
	opts.exec = $.extend(Object.create(null), {
		cd: ".",
		cmd: "node index.js",
	...
	// but it just morphs into the "workers" array
	if( $.is('object', opts.worker) ) {
		opts.workers = [ opts.worker ]
		delete opts.worker
	}
	// and then "worker" becomes read-only (see: "workers")
...
Herd.prototype.update = function() {
	...
	if( child == null ) return p.reject("No children to update")
...
Herd.prototype.workerRestart = function() {
	...
	$(self.workers).each(function(worker) {
		if( worker.process != null ) {
			...
			$.Promise.allChildrenOf(worker.process.pid).wait(function(err, pids) {
				if( err ) {
					return log("Failed to kill worker:", err)
				}
				log("Killing all working children:", pids)
				$(pids).each(function(pid) {
					Shell.exec("kill -15 " + pid)
					// will cause the worker to restart itself (as from un-natural death)
				})
			})
		} else {
...
Herd.prototype.rollingRestart = function(from, done) {
	...
	try {
		if( from < 0 ) {
			...
			log("Killing old process:", from)
			self.children[from].kill().wait(self.restart.gracePeriod, function(err) {
				if( err != null ) return fail(err)
				self.children[from].started.wait(function(err) {
					if( err != null ) return fail(err)
					self.rollingRestart(next, done)
				})
			})
...
Herd.prototype.killAll = function() {
	var self = this, p = $.Progress(self.children.length + self.workers.length);
	$(self.children).each(function(child) {
		if( child && child.process ) {
			$.Promise.allChildrenOf(child.process.pid).then(function(pids) {
				$(pids).each(function(pid) {
	...
	$(self.workers).each(function(worker) {
		if( worker && worker.process ) {
			$.Promise.allChildrenOf(worker.process.pid).then(function(pids) {
				$(pids).each(function(pid) {
...
Herd.prototype.getChildrenStatus = function() {
