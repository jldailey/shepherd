var $ = require('bling'),
	...
	Herd = function Herd(opts) {
		...
		log("Starting", self.exec.count, "children")
		...
		if( opts.workers.length > 0 ) {
			i = 0
			$(opts.workers).map(function(w) {
				log("Starting", w.count, "workers")
				var a = new Array(w.count)
				for( j = 0; j < w.count; j++) {
		...
		webRoute("get", "/workers", plain(function(req, res) {
			res.send(200, JSON.stringify(self.getWorkerStatus()))
		}))
...
Herd.defaults = function(opts) {
	...
	$(opts.workers).map(function(worker) {
...
Herd.prototype.workerRestart = function() {
	var self = this;
	$(self.workers).map(function(worker) {
		if( worker.process != null ) {
			self.log("Killing old worker:", worker.process.pid)
			worker.process.kill("SIGKILL")
		} else {
			self.log("Launching new worker...")
			worker.spawn()
		}
	})
}
