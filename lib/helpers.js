	...
	$.Promise.allChildrenOf(pid).wait(function(err, children) {
		var poll_port = function() {
			if( $.now - started > timeout ) {
				return p.fail("Waiting failed after a timeout of: " + timeout + "ms")
			}
			$.Promise.portOwner(port).wait(function (err, owner) {
				if( err != null ) p.fail(err)
				else if( children.indexOf(String(owner)) > -1 ) p.finish(owner)
...
$.Promise.allChildrenOf = function(pid) {
	...
	switch( Os.platform() ) {
		...
		case "linux":
			...
			break
		default:
			p.fail('unsupported platform: '+Os.platform())
	}
