	...
	try return p = $.Promise()
		...
		if psCache.has ps_cmd
			...
			attach_ports(ps_parse(output)).then ((procs) ->
				$(procs).zap "kill", (proc) -> (signal) -> Process.kill proc, signal
...
Process.kill = (pid, signal) -> Process.exec "kill -#{getSignalNumber signal} #{pid}"
...
Process.printTree = (proc, indent, spacer) ->
	...
	if proc.ports.length > 0 then ret += " [:" + proc.ports.join(", :") + "]"
