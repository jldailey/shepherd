	...
	start: ->
		try return @started
			...
			if ++@started.attempts > @opts.restart.maxAttempts
				...
				# The @started.timeout keeps track of how long we have been waiting for the @started promise to finish
				# For servers, this happens when the PORT is owned.
				# For workers, this happens as soon as the shell command finishes.
				clearTimeout @started.timeout
				@started.timeout = setTimeout (=> @started.attempts = 0), @opts.restart.maxInterval

				# Execute the shell command that will launch the new child.
				log "shell >" , cmd = "env #{@env()} bash -c 'cd #{@opts.cd} && #{@opts.command}'"
				@process = Shell.exec cmd, { silent: true, async: true }, $.identity
				# When the new child process exits, run our @onExit handler
				@process.on "exit", (err, signal) => @onExit err, signal

				# All data generated by the new child (via stdout, stderr) is captured and logged
				on_data = (prefix) => (data) =>
				...
				# If the Shell.exec above didn't produce a real process, then die.
				unless @process.pid then fail "no pid"

				# IMPORTANT NOTE: this function does not resolve @started on it's own,
